...
export default function BeritaContent() {
    ...
    const getListBerita = async () => {
        try {
            ...
            if (res.data.data.length > 5) {
                const recentNews = res.data.data.slice(0, 5);
                setRecentPost(recentNews);
            } else {
    ...
    const getData = async() => {
        try {
            const res = await axios.get(`${process.env.REACT_APP_BASE_URL}/api/web/news/detail/${searchParams.get('id')}`);
